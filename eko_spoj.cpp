...
//new woodcutting machine that can take down forests like wildfire. However, Mirko is only allowed to cut a single row of trees.

// Mirko‟s machine works as follows: Mirko sets a height parameter H (in metres), and the machine raises a 
//giant sawblade to that height and cuts off all tree parts higher than H (of course, trees not higher than H meters remain intact). 
...
bool isPossible(long long int n, long long int m, long long int arr[], long long int mid)
{
    long long int sum = 0;
...
int eko_spoj(long long int n, long long int m, long long int arr[])
{
